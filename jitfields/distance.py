...
def spline_distance_table(
    ...
    if isinstance(steps, int):
        ...
        steps = torch.as_tensor(
            steps, dtype=coeff.dtype, device=coeff.device
        ).flatten()
...
def mesh_distance_signed(
    ...
    faces : `(M, D) tensor[integer]`
        Mesh faces
    squared : bool
        Return the signed squared distance
    ...
    """
    d = MeshSDT.apply(loc, vertices, faces, out)
    if not squared:
        d = _unsquare_sdt(d)
    return d
...
def mesh_sdt_extra(
    ...
    """
    d, v = MeshSDTExtra.apply(loc, vertices, faces, out)
    if not squared:
        d = _unsquare_sdt(d)
    return d, v
...
def mesh_distance(
    ...
    dist : `(...) tensor`
        ...
        on the mesh (negative inside, positive outside)
    """
    d = MeshDT.apply(loc, vertices, faces, out)
    if not squared:
        d = d.sqrt()
    return d
...
class MeshSDT(torch.autograd.Function):

    @staticmethod
    def forward(ctx, loc, vertices, faces, out):
        ...
        if loc.requires_grad or vertices.requires_grad:
            ctx.save_for_backward(
                loc, vertices, faces, tree, normf, normv, norme
            )
        return out

    @staticmethod
    def backward(ctx, grad):
        fn_sdt = (cuda_dist.mesh_sdt_backward if grad.is_cuda else
                  cpu_dist.mesh_sdt_backward)

        if ctx.saved_tensors[0].requires_grad:
            gradcoord = torch.empty_like(ctx.saved_tensors[0])
        else:
            gradcoord = None
        if ctx.saved_tensors[1].requires_grad:
            gradvert = torch.zeros_like(ctx.saved_tensors[1])
        else:
            gradvert = None

        fn_sdt(gradcoord, gradvert, grad, *ctx.saved_tensors)
        return gradcoord, gradvert, None, None


class MeshSDTExtra(torch.autograd.Function):
...
class MeshDT(torch.autograd.Function):
    ...
    def forward(ctx, loc, vertices, faces, out):
        ...
        if loc.requires_grad or vertices.requires_grad:
            ctx.save_for_backward(loc, vertices, faces, tree)
        return out

    @staticmethod
    def backward(ctx, grad):
        fn_dt = (cuda_dist.mesh_dt_backward if grad.is_cuda else
                 cpu_dist.mesh_dt_backward)
        ...
        fn_dt(gradcoord, gradvert, grad, *ctx.saved_tensors)
        return gradcoord, gradvert, None, None
