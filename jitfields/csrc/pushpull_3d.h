...
struct PushPull<three, Z, BX, Z, BY, Z, BZ> {
              ...
              offset_t nc, offset_t osc, offset_t isc)
    {
        ...
        utils_z::index(y, nz, iz, fz);
        offset_t    i = ix * sx + iy * sy + iz * sz;
                       ...
                       offset_t osg, offset_t isg)
    {
        ...
        self::push(out, ginp, x, nx, isx, y, ny, isy, z, nz, isz, nc, osc, isc);
    }
                       ...
                       offset_t osg, offset_t isg)
    {
        ...
        gout[osg]     = static_cast<scalar_t>(0);
        gout[osg * 2] = static_cast<scalar_t>(0);
        self::pull(out, ginp, x, nx, sx, y, ny, sy, z, nz, sz, nc, osc, isc);
    }
...
struct PushPull<three, L, BX, L, BY, L, BZ> {
              ...
              offset_t nc, offset_t osc, offset_t isc)
    {
        ...
        utils_z::index(z, nz, iz0, iz1, wz0, wz1, fz0, fz1);
        ix0 *= sx; ix1 *= sx; iy0 *= sy; iy1 *= sy; iz0 *= sz; iz1 *= sz;
                       ...
                       offset_t osg, offset_t isg)
    {
        ...
        {
            offset_t ox0 = ix0 * osx, ox1 = ix1 * osx;
            offset_t oy0 = iy0 * osy, oy1 = iy1 * osy;
            offset_t oz0 = iz0 * osz, oz1 = iz1 * osz;
            ix0 *= isx; ix1 *= isx; iy0 *= isy; iy1 *= isy; iz0 *= isz; iz1 *= isz;
            offset_t i00 = iy0 + iz0;
            offset_t i01 = iy0 + iz1;
            offset_t i10 = iy1 + iz0;
            offset_t i11 = iy1 + iz1;
...
struct PushPull<three, IX, BX, IY, BY, IZ, BZ> {
              ...
              offset_t nc, offset_t osc, offset_t isc)
    {
        ...
        offset_t lx = utils_x::index(x, nx, ix, wx, fx);
        offset_t ly = utils_y::index(y, ny, iy, wy, fy);
        offset_t lz = utils_z::index(z, nz, iz, wz, fz);
