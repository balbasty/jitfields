...
inline __device__ size_t inbounds(size_t coord, size_t size) {
  return coord;
}
...
//     n --> n-2
template <typename size_t>
inline __device__ size_t reflect1c(size_t coord, size_t size) {
  if (size == 1) return 0;
...
//   n+1 --> n-1
template <typename size_t>
inline __device__ size_t reflect1s(size_t coord, size_t size) {
  if (size == 1) return 0;
  size_t size_twice = (size+1)*2;
...
//     n --> n-1
template <typename size_t>
inline __device__ size_t reflect2(size_t coord, size_t size) {
...
//     n --> 0
template <typename size_t>
inline __device__ size_t circular(size_t coord, size_t size) {
...
//   n+1 --> n-1
template <typename size_t>
inline __device__ size_t replicate(size_t coord, size_t size) {
...
template <typename size_t>
inline __device__ signed char inbounds(size_t coord, size_t size) {
...
template <typename size_t>
constexpr inline __device__ signed char constant(size_t coord, size_t size) {
...
template <typename size_t>
inline __device__ signed char periodic1(size_t coord, size_t size) {
  if (size == 1) return 1;
  size_t size_twice = (size+1)*2;
...
// Periodic sign change based on coordinates
template <typename size_t>
inline __device__ signed char periodic2(size_t coord, size_t size) {
  coord = (coord < 0 ? size - coord - 1 : coord);
...
template <typename size_t>
inline __device__ bool inbounds(size_t coord, size_t size) {
  return coord >= 0 && coord < size;
}

template <typename scalar_t, typename size_t>
inline __device__ bool inbounds(scalar_t coord, size_t size, scalar_t tol) {
  return coord >= -tol && coord < (scalar_t)(size-1)+tol;
}

template <typename scalar_t, typename offset_t>
inline __device__ scalar_t
get(const scalar_t * ptr, offset_t offset, signed char sign) {
...
template <> struct utils<type::Replicate> {
    ...
    template <typename size_t>
    static constexpr inline __device__ signed char sign(size_t coord, size_t size)
