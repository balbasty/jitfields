...
n = 8
few_vertices = [-i*360/n for i in range(n)]
few_vertices = map(lambda x: x*pi/180, few_vertices)
few_vertices = map(lambda x: [cos(x), sin(x)], few_vertices)
few_vertices = list(few_vertices)

few_edges = [[i, i+1] for i in range(n)]
few_edges[-1][-1] = 0

few_vertices = torch.as_tensor(few_vertices, dtype=torch.float32)
few_edges = torch.as_tensor(few_edges, dtype=torch.int32)

few_vertices *= 0.5

few_vertices = few_vertices.cuda()
...
for _ in range(128):
    ...
    vertices.grad = None
    d = mesh_distance(few_vertices, vertices, edges, squared=True).sum()
    print(d.item())
    d.backward()
    with torch.no_grad():
        vertices.detach().sub_(vertices.grad, alpha=1e-1)
