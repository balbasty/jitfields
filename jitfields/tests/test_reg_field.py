...
def test_matvec(device, dim):
    ...
    pred_absolute = torch.stack(
        [field_matvec(dim, basis[..., d], absolute=1)[..., d] for d in range(dim)],
        -1)
    ...
    pred_membrane = torch.stack(
        [field_matvec(dim, basis[..., d], membrane=1)[..., d] for d in range(dim)],
        -1)
    ...
    pred_bending = torch.stack(
        [field_matvec(dim, basis[..., d], bending=1)[..., d] for d in range(dim)],
        -1)
